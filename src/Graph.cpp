...
void Graph::initializeGraph(unsigned int nbNodes, unsigned int nbEdges, std::map<unsigned int,unsigned int>& lit2color,  std::vector<std::vector<unsigned int> >& neighbours){
    saucy_g = (saucy_graph*) malloc(sizeof (struct saucy_graph));
    saucy_g->colors = (int*) malloc(nbNodes * sizeof (int));
    for(unsigned int i = 0; i < lit2color.size(); i ++){
...
unsigned int Graph::getNbNodesFromGraph(){
    return (unsigned int) saucy_g->n;
}

unsigned int Graph::getNbEdgesFromGraph() {
    return (unsigned int) saucy_g->e;
}

unsigned int Graph::getColorOf(unsigned int node) {
    return saucy_g->colors[node];
}

unsigned int Graph::nbNeighbours(unsigned int node){
    return saucy_g->adj[node + 1] - saucy_g->adj[node];
}
unsigned int Graph::getNeighbour(unsigned int node, unsigned int nbthNeighbour){
    return saucy_g->edg[saucy_g->adj[node] + nbthNeighbour];
}
void Graph::setNodeToNewColor(unsigned int node) {
...
Graph::Graph(std::unordered_set<sptr<Clause>, UVecHash, UvecEqual>& clauses) {
  ...
  int n = 2 * nVars + clauses.size();
    ...
    std::map<unsigned int,unsigned int> lit2color{};

  // Initialize colors:
  for (unsigned int i = 0; i < 2 * nVars; ++i) {
      lit2color[i] = 0;
  }
  ...
  std::unordered_set<unsigned int> usedLits;
  for (auto cl : clauses) {
    for (auto lit : cl->lits) {
      usedLits.insert(lit);
...
Graph::Graph(std::unordered_set<sptr<Clause>, UVecHash, UvecEqual>& CNFClauses, std::unordered_set<sptr<Clause>, UVecHash, UvecEqual>& XORClauses) {

  int n = 2 * nVars + CNFClauses.size() + XORClauses.size();
  ...
  if (CNFClauses.size() > 0) {
    for (unsigned int i = 2 * nVars; i < 2 * nVars + CNFClauses.size(); ++i) {
      lit2color[i] = 1;
    }
    colorcount.push_back(CNFClauses.size());
  }

  if (XORClauses.size() > 0) {
    unsigned int color = (CNFClauses.size() == 0 ? 1 : 2);
    for (int i = 2 * nVars + CNFClauses.size(); i < n; ++i) {
      lit2color[i] = color;
    }
    colorcount.push_back(XORClauses.size());
  }
  ...
  // CNF clauses have as neighbors the literals occurring in them
  unsigned int c = 2 * nVars;
  ...
  // XOR clauses have as neighbors the literals occurring in them
...
Graph::Graph(std::unordered_set<sptr<PBConstraint>, UVecHash, UvecEqual>& constrs){
  ...
  unsigned int lastUsedColor=0;
  std::map<unsigned int,unsigned int> coeff2color;
  std::map<unsigned int,unsigned int> bound2color;
  auto getCoeffColor = [&coeff2color,&lastUsedColor,this] (unsigned int coeff)->unsigned int{
  ...
  auto getBoundColor = [&bound2color,this,&lastUsedColor] (unsigned int bound)->unsigned int{
  ...
  std::map<unsigned int,unsigned int> node2color;
  std::vector<std::vector<unsigned int> > neighbours(2 * nVars);

  auto getNewNode = [&neighbours, &node2color]()->unsigned int{
  ...
  std::map<std::pair<unsigned int,unsigned int>, unsigned int> litCoeff2node; //maps a lit and coefficient to a node (for node reuse and keeping graph small)
  auto getnode = [&node2color,this,&litCoeff2node,&getCoeffColor,&neighbours,&getNewNode] (unsigned int lit, unsigned int coeff)->unsigned int{
  ...
  for(auto c: constrs){
    unsigned int constraintNode = getNewNode();
...
unsigned int Graph::getNbNodes(){
        return getNbNodesFromGraph();
}

unsigned int Graph::getNbEdges() {
  return getNbEdgesFromGraph();
}

void Graph::setUniqueColor(unsigned int lit) {
    auto color = getColorOf(lit);
  unsigned int currentcount = colorcount[getColorOf(lit)];
  if (currentcount == 1) {
...
void Graph::setUniqueColor(const std::vector<unsigned int>& lits) {
  for (auto lit : lits) {
    setUniqueColor(lit);
  }
}
