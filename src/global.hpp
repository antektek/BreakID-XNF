...
// GLOBALS:
extern unsigned int nVars;
extern std::vector<unsigned int> fixedLits;
...
extern unsigned int nbConstraintsInProofLog;
...
inline bool sign(unsigned int lit) {
  return lit & 1;
}

inline unsigned int neg(unsigned int lit) {
  return lit ^ 1;
}

inline unsigned int encode(int lit) {
  return (lit > 0 ? 2 * (lit - 1) : 2 * (-lit - 1) + 1);
}

inline int decode(unsigned int lit) {
...
public:
  ...
  Clause(const std::set<unsigned int>& inLits) :
  ...
  void printAsRule(std::ostream& ostr, unsigned int falsevar);
...
private:
  ...
  std::map<unsigned int,unsigned int> terms; // maps lits to coefficients
  int weight;
...
public:
   ...
   */
  void addTerm(unsigned int lit, int coeff){
    if(terms.count(lit)!=0){
      coeff += terms[lit];
    }
    if(terms.count(neg(lit))!=0){
      unsigned int c = terms[neg(lit)];
      weight -= c;
      coeff -= terms[neg(lit)];
    }
    if(coeff<0){
  ...
  const std::map<unsigned int,unsigned int>& getTerms() const { return terms; }
...
public:
  ...
  int ruleType;
  std::vector<unsigned int> headLits;
  std::vector<unsigned int> bodyLits;
  int bound; //Only relevant for rules of type 2 and 5
  std::vector<int> weights;

  Rule(int inRuleType, const std::vector<unsigned int>& inHeads, const std::vector<unsigned int>& bodies, int inBound, std::vector<int>& inWeights) :
...
struct UVecHash {
  ...
  size_t operator()(const sptr<std::vector<unsigned int> > first) const {
...
template<class T>
void swapErase(std::vector<T>& vec, unsigned int index) {
  vec[index] = vec.back();
  vec.pop_back();
}
